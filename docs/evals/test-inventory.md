...
| admin schedule management | [schedule.utils.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/schedule.utils.test.ts), [schedule-form.schema.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/form/schedule-form.schema.test.ts) | code-graded | 캘린더 날짜 계산 / timezone 없는 월 key와 Seoul 기준 월간 조회 query / 일정 필터 / 종료일시 기준 지난 일정 판단 / 시작일시 정렬 / 백엔드 응답 매핑 / 일정 기간 표시 / 목록용 날짜 배지·월별 그룹 / 빈 장소·설명 fallback / 회장 일정 폼 스키마 / 외부 링크 payload 정규화·검증 |
