...
| admin home dashboard | `apps/DONGLE-ADMIN/src/app/(대시보드)/(총동연)/admin/admin-home-data.test.ts` | code-graded | 대시보드 집계 성공 반환 / API 실패 응답 정규화 / 네트워크 예외 발생 시 isError 정규화 |
| admin feedback | [admin-feedback-form.test.ts](../../apps/DONGLE-ADMIN/src/feature/feedback/admin-feedback-form.test.ts), [admin-feedback.action.test.ts](../../apps/DONGLE-ADMIN/src/feature/feedback/admin-feedback.action.test.ts) | code-graded | 카테고리 목록 / category·content validation / 허용되지 않은 category 값 거부 / 서비스 401 응답 세션 만료 처리 / 성공 응답 |
| admin shared action | [action-result.test.ts](../../apps/DONGLE-ADMIN/src/shared/action/action-result.test.ts), [zod-field-errors.test.ts](../../apps/DONGLE-ADMIN/src/shared/action/zod-field-errors.test.ts), [get-action-error-message.test.ts](../../apps/DONGLE-ADMIN/src/shared/action/get-action-error-message.test.ts), [get-service-error-message.test.ts](../../apps/DONGLE-ADMIN/src/shared/action/get-service-error-message.test.ts) | code-graded | typed action result helper / zod field error mapper / catch 블록 에러 메시지 추출 fallback / 서비스 API error message·detail 우선순위 |
| admin shared form | [date-picker-value.test.ts](../../apps/DONGLE-ADMIN/src/shared/form/date-picker-value.test.ts), [rhf-file-upload.test.ts](../../apps/DONGLE-ADMIN/src/shared/form/rhf-file-upload.test.ts), [use-action-form-submit.test.ts](../../apps/DONGLE-ADMIN/src/shared/form/use-action-form-submit.test.ts) | code-graded | date picker local date/datetime formatter / replace file upload cancel restore helper / action form submit runner |
| admin sentry capture | [capture-server-exception.test.ts](../../apps/DONGLE-ADMIN/src/lib/sentry/capture-server-exception.test.ts) | code-graded | extra에서 login_id/password/registrationKey/token류 필드 제거 |
...
| user form | [user-form.validation.test.ts](../../apps/DONGLE-ADMIN/src/feature/user/validation/user-form.validation.test.ts), [user-form.schema.test.ts](../../apps/DONGLE-ADMIN/src/feature/user/form/user-form.schema.test.ts), [change-account-form.action.test.ts](../../apps/DONGLE-ADMIN/src/feature/user/action/change-account-form.action.test.ts) | code-graded | validation / shared RHF server action schema / update payload mapper / 계정 변경 service 실패 분기와 cache invalidation |
...
| admin user management | [filterable-user-list.test.ts](../../apps/DONGLE-ADMIN/src/feature/user/components/filterable-user-list.test.ts), `apps/DONGLE-ADMIN/src/app/(대시보드)/(총동연)/admin/(메뉴)/(사용자관리)/user/page.test.ts`, [delete-user.action.test.ts](../../apps/DONGLE-ADMIN/src/feature/user/action/delete-user.action.test.ts) | code-graded | 검색 필터 로직 / 사용자 목록 조회 실패 안내 / 사용자 목록 service 예외 정규화 / 자기 계정 삭제 방지 |
| admin report management | [filterable-report-list.test.ts](../../apps/DONGLE-ADMIN/src/feature/report/components/filterable-report-list/filterable-report-list.test.ts) | code-graded | 검색 필터 로직(제목/raw HTML content) / 목록 조회 실패 안내와 작성하기 진입점 유지 |
...
| cross-app cache invalidation | [revalidate-tags.test.ts](../../apps/DONGLE-ADMIN/src/lib/server/revalidate-tags.test.ts), [route.test.ts](../../apps/DONGLE-CLIENT/src/app/api/revalidate/route.test.ts) | code-graded | ADMIN main-banner/club-schedule 태그 변경 시 CLIENT로 무효화 webhook 호출 prefix 필터·실패 무시 / CLIENT 측 시크릿 검증·tags 형식 검증·revalidateTag 호출 |
...
| admin schedule management | [schedule.utils.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/schedule.utils.test.ts), [schedule-form.schema.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/form/schedule-form.schema.test.ts), [use-club-schedule-form.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/form/use-club-schedule-form.test.ts), [schedule-form-dialog-state.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/components/schedule-form-dialog-state.test.ts), [schedule-list-item.test.tsx](../../apps/DONGLE-ADMIN/src/feature/schedule/components/schedule-list-item.test.tsx) | code-graded | 캘린더 날짜 계산 / timezone 없는 월 key와 Seoul 기준 월간 조회 query / 일정 필터 / 시작·종료일시 기준 일정 상태 판단 / 진행 중 분리와 현재 기준 가까운 순 정렬 / 백엔드 응답 매핑 / 공통 일정 라벨 매핑 / 일정 기간 표시 / 날짜 아젠다 없는 월별 그룹 / 관리자 일정 목록 동아리명 우선 표시 / 빈 장소·설명 fallback / 회장 일정 폼 스키마 / 일정 폼 다이얼로그 닫힘 전환 중 수정 대상 표시 유지 / 외부 링크 payload 정규화·검증 / 공통·동아리 일정 생성·수정 submit action 분기 |
| club schedule service | [club.schedule.service.test.ts](../../packages/service/src/club/club.schedule.service.test.ts), [schedule.action.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/action/schedule.action.test.ts) | code-graded | 사용자/회장/관리자 일정 엔드포인트 / public 전체 일정 엔드포인트 / 관리자 공통 일정 생성·수정 엔드포인트 / query string / 공개 일정 cache tag / 회장 일정 생성·수정 실패 응답 분기 / 관리자 일정 수정·공개 상태 변경 동아리 범위 tag / 관리자 삭제 동아리 범위 tag / 관리자 삭제 전 상세 조회 실패 시 삭제 중단 / 삭제 action 실패 응답 분기 |
...
| main banner display | [get-display-banner-image-urls.test.ts](../../packages/service/src/main-banner/get-display-banner-image-urls.test.ts), [main-banner.service.test.ts](../../packages/service/src/main-banner/main-banner.service.test.ts), [main-banner-datetime.test.ts](../../apps/DONGLE-ADMIN/src/feature/main-banner/utils/main-banner-datetime.test.ts), [main-banner-form.schema.test.ts](../../apps/DONGLE-ADMIN/src/feature/main-banner/form/main-banner-form.schema.test.ts), [delete-main-banner.action.test.ts](../../apps/DONGLE-ADMIN/src/feature/main-banner/action/delete-main-banner.action.test.ts) | code-graded | display helper / link normalization / blank link null payload / admin-user endpoint split / admin detail endpoint / admin detail exception normalization / public/no-store cache policy / admin form datetime formatter / shared RHF server action schema / delete action result and cache invalidation |
| api msw helpers | [api-path.test.ts](../../packages/api/src/mocks/api-path.test.ts), [fixtures.test.ts](../../packages/api/src/mocks/fixtures.test.ts) | code-graded | `/v1` path 패턴 / `NEXT_PUBLIC_USE_MSW` 판정 / auth passthrough path / Club·User·ClubReport fixture DTO 형태 |
| api token refresh retry | [make-request.test.ts](../../packages/api/src/make-request.test.ts) | code-graded | auth retry guard helper |
| api instance response parsing | [instance.test.ts](../../packages/api/src/instance.test.ts) | code-graded | API 응답은 성공/실패 모두 throw 없이 JSON 바디를 그대로 반환하고, JSON 파싱 불가 응답은 합성 실패 응답으로 정규화 |
| api browser instance | [browser-instance.test.ts](../../packages/api/src/browser-instance.test.ts) | code-graded | 같은 origin 프록시 라우트 호출 시 JSON/FormData body 직렬화, Content-Type 자동 설정, 구조화 실패와 JSON 파싱 불가 응답 모두 throw 없이 정규화 |
| shared normalization | [string.test.ts](../../packages/utils/src/string.test.ts), [club.test.ts](../../packages/utils/src/club.test.ts), [phone.test.ts](../../packages/utils/src/phone.test.ts), [date.test.ts](../../packages/utils/src/date.test.ts), [url.test.ts](../../packages/utils/src/url.test.ts) | code-graded | trim/nullable/club tags/phone/date/url normalization and Seoul 기준 display/request/month-key/timestamp helper |
...
| client seo | [site-json-ld.test.ts](../../apps/DONGLE-CLIENT/src/lib/site-json-ld.test.ts), [page-metadata.test.ts](../../apps/DONGLE-CLIENT/src/lib/page-metadata.test.ts), [club-page-metadata.test.ts](../../apps/DONGLE-CLIENT/src/lib/club-page-metadata.test.ts), [report-page-metadata.test.ts](../../apps/DONGLE-CLIENT/src/lib/report-page-metadata.test.ts) | code-graded | WebSite / Organization JSON-LD graph / page OG·Twitter metadata helper / club description fallback / report description·OG metadata |
| client search | [use-club-filters.test.ts](../../apps/DONGLE-CLIENT/src/hooks/use-club-filters.test.ts), [page.test.ts](../../apps/DONGLE-CLIENT/src/app/page.test.ts), [club-list-section.test.tsx](../../apps/DONGLE-CLIENT/src/components/main/club-list-section.test.tsx) | code-graded | filtering / recruiting count helper / summary / query string parser-builder / 메인 동아리 목록 service 예외 정규화 / 메인 동아리 목록 조회 실패 안내 |
