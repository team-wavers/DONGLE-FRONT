...
| admin schedule management | [schedule.utils.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/schedule.utils.test.ts), [schedule-form.schema.test.ts](../../apps/DONGLE-ADMIN/src/feature/schedule/form/schedule-form.schema.test.ts) | code-graded | 캘린더 날짜 계산 / 일정 필터 / 시작일시 정렬 / 백엔드 응답 매핑 / 일정 기간 표시 / 빈 장소·설명 fallback / 회장 일정 폼 스키마 / 외부 링크 payload 정규화·검증 |
