...
- 진행 중이 아닌 나머지 일정은 시작일시가 최신인 순으로 정렬되어야 한다.
...
- 사용자 동아리 상세 일정 목록은 진행 중 일정을 별도 상단 섹션으로 표시하고 나머지는 월별 그룹 없이 최신 일정 섹션으로 표시해야 한다.
- 사용자 동아리 상세 최신 일정 목록은 초기 6개만 표시하고 남은 일정이 있으면 더보기 버튼으로 추가 노출해야 한다.
