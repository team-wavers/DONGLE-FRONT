...
- 진행 중이 아닌 나머지 일정은 현재 시점과 가까운 순으로 정렬한 뒤 해당 순서에 따라 Seoul 기준 시작 월별 섹션으로 묶어야 한다.
...
- 사용자 동아리 상세 월별 일정 목록은 초기 6개만 표시하고 남은 일정이 있으면 더보기 버튼으로 추가 노출해야 한다.
- 사용자 동아리 상세 일정 목록은 날짜 아젠다 없이 각 일정 항목 안에 일정 기간을 독립적으로 표시해야 한다.
- 사용자 동아리 상세 일정은 진행중, 예정, 마감 상태를 배지로 표시해야 한다.
- 사용자 동아리 상세 일정 카드의 타입별 색상 강조는 일정 유형 태그에만 적용하고 카드 왼쪽 선과 날짜 배지는 중립 색상으로 표시해야 한다.
