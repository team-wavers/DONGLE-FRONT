...
## 3) 서버 오케스트레이션 레이어 (RSC + Server Action + Route Handler)

대상: Next.js **서버 컴포넌트(RSC)**, `"use server"` **Server Action**, `app/api/**/route.ts` **Route Handler**

> 기술적으로 Server Action, RSC, Route Handler는 다르지만, **역할 관점(서버에서 서비스 호출 결과를 조립하고 정책을 결정)**에서는 같은 계층으로 본다.
