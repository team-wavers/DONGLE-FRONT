...
- TDD를 적용할 정도의 규모 작업은 구현과 검증을 마친 뒤, 종료 보고 전에 서브에이전트에게 변경된 코드 리뷰를 맡긴다.
- 서브에이전트 리뷰는 버그, 회귀 위험, 테스트 공백, 계약 위반을 우선 확인하게 한다.
...
- 종료 전 `pnpm verify:fast` 결과를 확인한다.
...
- `pnpm verify:fast` 실패는 우선 code/product 문제로 본다.
