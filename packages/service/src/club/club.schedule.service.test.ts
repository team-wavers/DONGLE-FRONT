...
describe("club schedule service endpoints", () => {
    ...
    test("사용자 공개 일정 목록은 공개 엔드포인트를 호출하고 응답 result를 반환한다", async () => {
        fetchInstanceMock.get.mockResolvedValueOnce({ isSuccess: true, result: [{ id: 1 }] });
    ...
    test("회장 일정 목록은 status query를 포함해 호출하고 응답 result를 반환한다", async () => {
        fetchInstanceMock.get.mockResolvedValueOnce({ isSuccess: true, result: [{ id: 2 }] });
    ...
    test("관리자 일정 목록은 query를 포함해 호출하고 응답 result를 반환한다", async () => {
        fetchInstanceMock.get.mockResolvedValueOnce({ isSuccess: true, result: [{ id: 4 }] });
    ...
    test("관리자 캘린더 일정은 기간 query를 포함해 호출하고 응답 result를 반환한다", async () => {
        fetchInstanceMock.get.mockResolvedValueOnce({ isSuccess: true, result: [{ id: 3 }] });
    ...
    test("관리자 일정 상세는 단건 엔드포인트를 호출한다", async () => {
        fetchInstanceMock.get.mockResolvedValueOnce({ isSuccess: true, result: { id: 7 } });
