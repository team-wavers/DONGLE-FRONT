/**
 * 사용자 생성/수정 폼 공통 검증
 */

export interface UserFormFieldErrors {
    name?: string;
    login_id?: string;
    password?: string;
    role?: string;
    phone?: string;
}

export const USER_FORM_MESSAGES = {
    nameRequired: "이름을 입력해주세요",
    loginIdRequired: "로그인 ID를 입력해주세요",
    loginIdEnglishOnly: "로그인 ID는 영문만 입력 가능합니다",
    passwordRequired: "비밀번호를 입력해주세요",
    passwordMinLength: "비밀번호는 4자 이상이어야 합니다",
    passwordEnglishOnly: "비밀번호는 영문만 입력 가능합니다",
    roleRequired: "역할을 선택해주세요",
    phoneRequired: "전화번호를 입력해주세요",
    phoneInvalid: "올바른 휴대폰 번호 형식이 아닙니다 (예: 010-1234-5678)",
} as const;

const ENGLISH_ONLY_REGEX = /^[a-zA-Z]+$/;

export function isValidPhoneNumber(phoneNumber: string): boolean {
    const cleaned = phoneNumber.replace(/\s/g, "");
    const phoneRegex = /^01[0-9]-?\d{3,4}-?\d{4}$/;
    return phoneRegex.test(cleaned);
}

export function isEnglishOnly(value: string): boolean {
    return ENGLISH_ONLY_REGEX.test(value);
}

export function validateUserName(name: string): string | undefined {
    if (!name || name.trim() === "") return USER_FORM_MESSAGES.nameRequired;
    return undefined;
}

export function validateLoginId(loginId: string): string | undefined {
    const trimmed = (loginId ?? "").trim();
    if (!trimmed) return USER_FORM_MESSAGES.loginIdRequired;
    if (!isEnglishOnly(trimmed)) return USER_FORM_MESSAGES.loginIdEnglishOnly;
    return undefined;
}

export function validatePassword(password: string, required: boolean): string | undefined {
    const p = (password ?? "").trim();
    if (!p) return required ? USER_FORM_MESSAGES.passwordRequired : undefined;
    if (p.length < 4) return USER_FORM_MESSAGES.passwordMinLength;
    if (!isEnglishOnly(p)) return USER_FORM_MESSAGES.passwordEnglishOnly;
    return undefined;
}

export function validateUserRole(role: string): string | undefined {
    if (!role || (role !== "admin" && role !== "president")) return USER_FORM_MESSAGES.roleRequired;
    return undefined;
}

export function validateUserPhone(phone: string): string | undefined {
    const trimmed = (phone ?? "").trim();
    if (!trimmed) return USER_FORM_MESSAGES.phoneRequired;
    if (!isValidPhoneNumber(trimmed)) return USER_FORM_MESSAGES.phoneInvalid;
    return undefined;
}
